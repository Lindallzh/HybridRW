...
int main(int argc, char* argv[]){
    ...
    for(int i=1; i<argc ; i++){
      ...
      //  else if (input.compare("--number") == 0) number = atoi(argv[++i]);
    }
  ...
  // {
    ...
    if(algorithm.compare("ns") == 0){
      ...
      // args[1] = args[1] + "_" +to_string(cnt)+"_" + p_str + "_" + q_str;
        outputdir = make_directory(args);
        if (file_exist(outputdir + "sampled_graph.txt")){
            cout << "Already Exist" << endl;
            return 0;
        ...
        cout << "Run RW" << endl;
       // args[1] = args[1] + "_"  +to_string(cnt)+"_"+ to_string(maxlength);
	   args[1] = args[1] + "_"+ to_string(maxlength);
        ...
        final = algo->run_he_node_nonback(target_portion);
  
        free(algo);
        algo = NULL;
    }
        else if(algorithm.compare("hrw") == 0){
        cout << "Run RW" << endl;
       // args[1] = args[1] + "_"  +to_string(cnt)+"_"+ to_string(maxlength);
	   args[1] = args[1] + "_"+ to_string(maxlength);
        //"_" +to_string(cnt)
        ...
        Algorithm_RW *algo = new Algorithm_RW(eval_opt, algo_opt, outputdir, graph, restart, maxlength); 
       // final = algo->run(target_portion);
       // final = algo->run_he_node(target_portion);
        final = algo->run_he_node_nonback(target_portion);
