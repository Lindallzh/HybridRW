...
HSet::HSet(set<int> subhypergraph, HyperGraph *graph, string eval_opt) {
    ...
    this->node_degree_size.resize(graph->number_of_nodes);
...
/*HSet::HSet(vector<int> subhypergraph, HyperGraph *graph, string eval_opt) {
    this->hypergraph_masking.resize(graph->number_of_hedges, false);
    for (auto h : subhypergraph){
        this->hypergraph_masking[h] = true;
    }
...
/*vector<int> HSet::get_nodeset(void){
    vector<int> nodeset;
    for (int h = 0 ; h < (int)nodeset_masking.size() ; h++){
        if (nodeset_masking[h]) nodeset.push_back(h);
    }
...
void HSet::calculate_distribution_rw(HyperGraph *graph, map<int,int> add_size, string eval_opt){
    vector<int> hyperedgeset = get_hyperedgeset();
   // attr->set_attribute_rw(graph->node2hyperedge, graph->hyperedge2node, hyperedgeset, add_size, node_degree, node_degree_size,eval_opt);
}

void HSet::calculate_distribution_node_rw(HyperGraph *graph, set<int> add_node, map<int,int> add_node_size, map<int,int> add_size, string eval_opt){
    vector<int> hyperedgeset = get_hyperedgeset();
     vector<int> nodeset;
    for (auto h : add_node)
    {
    ...
    sort(nodeset.begin(), nodeset.end());
    attr->set_attribute_rw(graph->node2hyperedge, graph->hyperedge2node, hyperedgeset, add_size, node_degree, node_degree_size,eval_opt,nodeset,add_node_size);

   // attr->set_attribute_deg_rwrw(graph->node2hyperedge,nodeset,add_node_size);
}
...
//to_textfile
void HSet::evaluate_rw(Attribute *target_attr){
    for (auto dist : attr->attr_list_rw){
        string dist_name = dist.first;
        auto dist1 = dist.second;
        auto dist2 =  target_attr->attr_list[dist_name];
        evaluation[dist_name] = this->get_Dstat_rw(dist1, dist2, dist_name);
    }
    double tmp = 0.0;
    for (auto eval : evaluation){
        tmp += eval.second;
...
double HSet::get_Dstat_rw(unordered_map<int,double> &dist, unordered_map<int,long long> &dist_target, string eval_name){
    vector<int> key_list;
    vector<int>::iterator it;
    double cumul_dist_v = 0.0, cumul_dist_t_v = 0.0;
    double sum_dist = 0.0, sum_dist_t = 0.0;
    ...
    for (auto key : key_list){
        if (sum_dist != 0){
            cumul_dist_v += (dist[key] / sum_dist);
        }
        if (sum_dist_t != 0){
            cumul_dist_t_v += ((double)dist_target[key] / sum_dist_t);
        }
